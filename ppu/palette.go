...
func (P *Palette) IsSpriteMirror(addr uint) bool {
	return (addr == 0x10) || (addr == 0x14) || (addr == 0x18) || (addr == 0x1c)
}
func (P *Palette) IsBackgroundMirror(addr uint) bool {
...
func (P *Palette) GetPaletteAddr(addr uint) uint {
	var mirrorDowned = uint((addr & 0xFF) % 0x20)
...
func (P *Palette) Write(addr uint, data byte) {
